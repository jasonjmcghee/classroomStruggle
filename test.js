...
function updateName(key, newSize, oldSize) {
  $("#keyname").text(key);
...
function percentChange(newSize, oldSize) {
	...
	var sign;
	var change;
	var newOverOld = ((newSize*100)/(oldSize*100));
	var percentOfOld = (Math.floor(newOverOld*100))/100;
...
function shade(diff) {
  ...
  if (diff < 1) currDiff = 100;
  else currDiff = 150+50*diff;
  if (currDiff > 255) currDiff = 255;
  console.log("CurrDiff "+currDiff);
  return "rgb("+currDiff+",0,0)";
}
  ...
  for (var key in data) {
    if (data.hasOwnProperty(key)) {
      ...
      if (oldSize < newSize) {
        ...
        .attr("onmouseover", "updateName(\""+key+"\", "+newSize+", "+oldSize+")")
        .style("fill", "beige")
